...
const AdminDashboard = ({ user }) => {
  ...
  const handleSetAdminStatus = async (userId, isAdmin) => {
    if (user.id === userId) {
      setPopup({
        isVisible: true,
        message: "Non puoi modificare il tuo stato di amministratore.",
        type: "error",
      });
      return;
    }
    ...
    if (
      ...
      try {
        ...
        setPopup({
          isVisible: true,
          message: "Stato amministratore aggiornato con successo!",
          type: "success",
        });
        fetchUsers();
      } catch (err) {
  ...
  const renderProductPageNumbers = () => {
    ...
    return pageNumbers.map((number) => (
      <button
        ...
        className={
          productCurrentPage === number ? "page-number active" : "page-number"
        }
      >
        {number}
      </button>
  ...
  const renderUserPageNumbers = () => {
    ...
    return pageNumbers.map((number) => (
      <button
        ...
        className={
          userCurrentPage === number ? "page-number active" : "page-number"
        }
      >
        {number}
      </button>
      ...
      <div className="admin-dashboard">
            ...
            {currentOrders.map((order) => (
              <li key={order.order_id}>
                <div className="order-summary">
                  <span>
                    Ordine #{order.order_id} - Totale: €{order.total_amount} -
                    Stato: {order.status}
                  </span>
                ...
                {openOrderId === order.order_id && (
                  <div className="order-details">
                    ...
                    <p>
                      Nome: {order.customer_first_name}{" "}
                      {order.customer_last_name}
                    </p>
                    <p>Email: {order.customer_email}</p>
                    <p>
                      Indirizzo: {order.customer_address}, {order.customer_city}
                      , {order.customer_province}, {order.customer_zip_code},{" "}
                    ...
                    <ul>
                      {order.items.map((item, index) => (
                        <li key={index}>
                          - {item.product_name} ({item.size}, {item.language}) x{" "}
        ...
        {filteredOrders.length > ordersPerPage && (
          <div className="pagination">
            ...
            <div className="page-numbers">{renderOrderPageNumbers()}</div>
            <button
              onClick={() =>
        ...
        {products.length > productsPerPage && (
          <div className="pagination">
            <button
              onClick={() =>
                setProductCurrentPage((prev) => Math.max(1, prev - 1))
              }
            ...
            <div className="page-numbers">{renderProductPageNumbers()}</div>
            <button
              onClick={() =>
                  ...
                  {user.id !== userItem.id && (
                    <>
                      ...
                      <button
                        ...
                        onClick={() =>
                          handleSetAdminStatus(userItem.id, userItem.is_admin)
                        }
                      >
                        {userItem.is_admin
                          ? "Declassa a Utente"
        ...
        {users.length > usersPerPage && (
          <div className="pagination">
            ...
            <div className="page-numbers">{renderUserPageNumbers()}</div>
            <button
              onClick={() =>
                setUserCurrentPage((prev) => Math.min(totalUserPages, prev + 1))
              }
