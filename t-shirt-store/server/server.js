...
app.post("/api/register", async (req, res) => {
  ...
  if (!firstName || !lastName || !email || !password) {
    return res
      .status(400)
      .json({ error: "Nome, cognome, email e password sono richiesti." });
  }
  ...
  try {
    ...
    if (existingUser.rows.length > 0) {
      return res
        .status(409)
        .json({ error: "Un utente con questa email è già registrato." });
    }
...
app.post("/api/password-reset", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Errore nella reimpostazione della password." });
  }
});
...
app.post("/api/products", authenticateAdmin, async (req, res) => {
  try {
    ...
    console.log("Dati ricevuti per nuovo prodotto:", {
      name,
      description,
      price,
      sizes,
    ...
    if (isNaN(price)) {
      return res
        .status(400)
        .json({ error: "Il prezzo deve essere un numero valido." });
    }
...
app.post("/api/orders", async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Ordine creato con successo!", orderId: orderId });
  } catch (err) {
...
app.get("/api/orders/:orderId/items", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Errore nel recupero degli articoli dell'ordine" });
  }
});
...
app.delete("/api/admin/orders/:id", authenticateAdmin, async (req, res) => {
  ...
  try {
    ...
    res.json({
      message: "Ordine e articoli correlati eliminati con successo.",
    });
  } catch (err) {
    await client.query("ROLLBACK");
    ...
    res
      .status(500)
      .json({ error: "Errore durante l'eliminazione dell'ordine" });
  } finally {
    client.release();
  }
});
